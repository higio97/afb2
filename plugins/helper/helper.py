...
class Helper:
    def __init__(self, bot: Client, message: Message):
        ...
        self.username = (
            "-" if not self.message.from_user.username else "@" + self.message.from_user.username
        )
    ...
    async def cek_langganan_channel(self, user_id: int):
        ...
        channels = [config.channel_1, config.channel_2, config.channel_3]
        statuses = [
    ...
    async def pesan_langganan(self):
        ...
        markup = InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("🔗 Channel Partnership", url=link_3)],
                [
                    InlineKeyboardButton("💌 Channel base", url=link_1),
                    InlineKeyboardButton("🗣️ Group base", url=link_2),
                ],
                [
                    InlineKeyboardButton(
                        "♻️ Coba lagi", url=f"https://t.me/{self.bot.username}?start=start"
                    )
                ],
            ]
        )
        await self.bot.send_photo(
            self.user_id,
            photo=config.pic_nonmember,
            caption=config.pesan_join,
    ...
    async def send_to_channel_log(self, type: str = None, link: str = None):
        if type == "log_daftar":
            ...
            pesan = "Jangan Lupa Main  @deltaxuniverse"
        await self.bot.send_message(
            config.channel_log, pesan, enums.ParseMode.HTML, disable_web_page_preview=True
        )

    ...
    if msg.chat.type == enums.ChatType.PRIVATE:
        ...
        if not database.get_data_bot(client.id_bot).bot_status:
            ...
            if member.status in status:
                return await client.send_message(
                    uid, "<i>Saat ini bot sedang dinonaktifkan</i>", enums.ParseMode.HTML
                )
        ...
        if command != None:
            if command == "/start":  # menampilkan perintah start
                ...
                return await belicoin_handler(client, msg)
            ...
            x = re.search(rf"(?:^|\s)({config.hastag})", command.lower())
            if x:
                # print('kaa')
                    ...
                    try:
                        ...
                        if tipe.type == enums.ChatType.BOT:
                            return await msg.reply(
                                "Terdeteksi username bot tidak dapat mengirim menfess"
                            )
                        elif tipe.type == enums.ChatType.CHANNEL:
                            return await msg.reply(
                                "Terdeteksi username channel/grup tidak dapat mengirim menfess"
                            )
                        ...
                        pass
                z = re.search(
                    r"(?:^|\s)(t\.me|telegram\.me)\/([-a-zA-Z0-9@:%._\+~#=]{1,256})",
                    command.lower(),
                )
                if z:
                    try:
                        ...
                        return await msg.reply(
                            "Terdeteksi link channel/grup/tautan tidak dapat mengirim menfess"
                        )
                u = re.search(
                ...
                if u:
                    try:
                        ...
                        if tipe.type == enums.ChatType.BOT:
                            return await msg.reply(
                                "Terdeteksi link bot tidak dapat mengirim menfess"
                            )
                        elif tipe.type == enums.ChatType.CHANNEL:
                            return await msg.reply(
                                "Terdeteksi link channel/grup tidak dapat mengirim menfess"
                            )
                        elif tipe.type == enums.ChatType.SUPERGROUP:
                            return await msg.reply(
                                "Terdeteksi link channel/grup tidak dapat mengirim menfess"

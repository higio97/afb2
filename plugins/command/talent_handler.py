...
async def talent_handler(client: Client, msg: types.Message):
    ...
    if len(talent) == 0:
        return await msg.reply(
            "<b>Saat ini tidak ada talent yang tersedia.</b>", True, enums.ParseMode.HTML
        )
    else:
        for uid in talent:
    ...
    if y:
        ...
        if await db.cek_user_didatabase():
            ...
            if member.status in status:
                return await msg.reply_text(
                    text=f"❌<i>Terjadi kesalahan, <a href='tg://user?id={str(target)}'>user</a> adalah seorang {member.status.upper()} tidak dapat menjadikan talent</i>",
                    quote=True,
                    parse_mode=enums.ParseMode.HTML,
                )
            try:
                ...
                nama = await helper.escapeHTML(
                    a.first_name if not a.last_name else a.first_name + " " + a.last_name
                )
    ...
    if x:
        ...
        if await db.cek_user_didatabase():
            ...
            if member.status == "talent":
                ...
                return await msg.reply_text(
                    text=f"<i><a href='tg://openmessage?user_id={str(target)}'>User</a> bukan seorang talent</i>",
                    quote=True,
                    parse_mode=enums.ParseMode.HTML,
                )
        else:
...
async def rate_talent_handler(client: Client, msg: types.Message):
    if re.search(r"^[\/]rate(\s|\n)*$", msg.text or msg.caption):
        return await msg.reply_text(
            text="perintah salah, gunakan perintah /rate id untuk memberikan rating kepada talent",
            quote=True,
            parse_mode=enums.ParseMode.HTML,
        )
    ...
    if x:
        ...
        if target in db_bot.talent:
            if my_coin <= config.biaya_talent:
                return await msg.reply(
                    f"coin kamu kurang untuk memberikan rating ke talent fwb. biaya rate adalah {str(config.biaya_talent)} coin",
                    True,
                )
            ...
            if my_coin <= config.biaya_daddy_sugar:
                return await msg.reply(
                    f"coin kamu kurang untuk memberikan rating ke talent fwb. biaya rate adalah {str(config.biaya_daddy_sugar)} coin",
                    True,
                )
            ...
            if my_coin <= config.biaya_moansgirl:
                return await msg.reply(
                    f"coin kamu kurang untuk memberikan rating ke talent fwb. biaya rate adalah {str(config.biaya_moansgirl)} coin",
                    True,
                )
            ...
            if my_coin <= config.biaya_moansboy:
                return await msg.reply(
                    f"coin kamu kurang untuk memberikan rating ke talent fwb. biaya rate adalah {str(config.biaya_moansboy)} coin",
                    True,
                )
            ...
            if my_coin <= config.biaya_gfrent:
                return await msg.reply(
                    f"coin kamu kurang untuk memberikan rating ke talent fwb. biaya rate adalah {str(config.biaya_gfrent)} coin",
                    True,
                )
            ...
            if my_coin <= config.biaya_bfrent:
                return await msg.reply(
                    f"coin kamu kurang untuk memberikan rating ke talent fwb. biaya rate adalah {str(config.biaya_bfrent)} coin",
                    True,
                )

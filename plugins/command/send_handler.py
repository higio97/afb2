...
async def send_with_pic_handler(client: Client, msg: types.Message, key: str, hastag: list):
    ...
    if msg.text or msg.photo or msg.video or msg.voice:
        ...
        kirim = await client.send_photo(
            config.channel_1, picture, caption, caption_entities=entities
        )
    ...
    if msg.text or msg.photo or msg.video or msg.voice:
        if msg.photo and not db_bot.photo:
            if db_user.status == "member" or db_user.status == "talent":
                return await msg.reply(
                    "Tidak bisa mengirim photo, karena sedang dinonaktifkan oleh admin", True
                )
        elif msg.video and not db_bot.video:
            if db_user.status == "member" or db_user.status == "talent":
                return await msg.reply(
                    "Tidak bisa mengirim video, karena sedang dinonaktifkan oleh admin", True
                )
        elif msg.voice and not db_bot.voice:
            if db_user.status == "member" or db_user.status == "talent":
                return await msg.reply(
                    "Tidak bisa mengirim voice, karena sedang dinonaktifkan oleh admin", True
                )
    ...
    if re.search(r"^[\/]tf_coin\s(\d+)(\s(\d+))?", msg.text or msg.caption):
        ...
        if my_coin >= int(coin):
            ...
            if await db_target.cek_user_didatabase():
                ...
                nama = (
                    "Admin"
                    if anu.status == "owner" or anu.status == "admin"

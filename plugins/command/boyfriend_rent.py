...
async def bf_rent_handler(client: Client, msg: types.Message):
    ...
    if len(bf_rent) == 0:
        return await msg.reply(
            "<b>Saat ini tidak ada Jasa Pembuatan Bot yang tersedia.</b>",
            True,
    ...
    if y:
        ...
        if await db.cek_user_didatabase():
            ...
            if member.status in status:
                return await msg.reply_text(
                    text=f"❌<i>Terjadi kesalahan, <a href='tg://user?id={str(target)}'>user</a> adalah seorang {member.status.upper()} tidak dapat ditambahkan menjadi Boyfriend rent</i>",
                    quote=True,
                    parse_mode=enums.ParseMode.HTML,
                )
            try:

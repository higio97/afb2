...
async def moans_boy_handler(client: Client, msg: types.Message):
    ...
    if len(moans_boy) == 0:
        return await msg.reply(
            "<b>Saat ini tidak ada Moans boy yang tersedia.</b>", True, enums.ParseMode.HTML
        )
    else:
    ...
    if y:
        ...
        if await db.cek_user_didatabase():
            ...
            if member.status in status:
                return await msg.reply_text(
                    text=f"❌<i>Terjadi kesalahan, <a href='tg://user?id={str(target)}'>user</a> adalah seorang {member.status.upper()} tidak dapat ditambahkan menjadi Moans boy</i>",
                    quote=True,
                    parse_mode=enums.ParseMode.HTML,
                )
            try:

...
async def daddy_sugar_handler(client: Client, msg: types.Message):
    ...
    if len(daddy_sugar) == 0:
        return await msg.reply(
            "<b>Saat ini tidak ada daddy sugar yang tersedia.</b>", True, enums.ParseMode.HTML
        )
    else:
    ...
    if y:
        ...
        if await db.cek_user_didatabase():
            ...
            if member.status in status:
                return await msg.reply_text(
                    text=f"❌<i>Terjadi kesalahan, <a href='tg://user?id={str(target)}'>user</a> adalah seorang {member.status.upper()} tidak dapat ditambahkan menjadi daddy sugar</i>",
                    quote=True,
                    parse_mode=enums.ParseMode.HTML,
                )
            try:

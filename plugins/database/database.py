...
class Database:
    ...
    async def transfer_coin(
        ...
        b = mycol.update_one(
            {"coin": coin_awal_target_full}, {"$set": {"coin": f"{diterima}_{id_target}"}}
        )
    ...
    async def banned_user(self, id_banned: int, id_bot: int, alasan: str):
        ...
        last_status = self.get_data_pelanggan().status_full
        mycol.update_one({"status": last_status}, {"$set": {"status": f"banned_{str(id_banned)}"}})
    ...
    async def unban_user(self, id_banned: int, id_bot: int):
        ...
        last_status = self.get_data_pelanggan().status_full
        mycol.update_one({"status": last_status}, {"$set": {"status": f"member_{str(id_banned)}"}})
        mycol.update_one(last_data, {"$set": {"ban": new_data}})

    async def tambah_talent(self, id_talent: int, id_bot: int, nama: str):
        last_data = {"talent": self.get_data_bot(id_bot).talent}
        ...
        last_status = self.get_data_pelanggan().status_full
        mycol.update_one({"status": last_status}, {"$set": {"status": f"talent_{str(id_talent)}"}})
    ...
    async def hapus_talent(self, id_talent: int, id_bot: int):
        ...
        last_status = self.get_data_pelanggan().status_full
        mycol.update_one({"status": last_status}, {"$set": {"status": f"member_{str(id_talent)}"}})
        mycol.update_one(last_data, {"$set": {"talent": new_data}})
    ...
    async def rate_talent(self, id_talent: str, id_bot: int, coin: int):
        ...
        mycol.update_one(
            {"coin": f"{last_coin}_{self.user_id}"}, {"$set": {"coin": f"{coin}_{self.user_id}"}}
        )
        mycol.update_one(last_data, {"$set": {"talent": new_data}})

    async def tambah_sugar_daddy(self, id_talent: int, id_bot: int, nama: str):
        last_data = {"daddy_sugar": self.get_data_bot(id_bot).daddy_sugar}
        ...
        last_status = self.get_data_pelanggan().status_full
        mycol.update_one(
            {"status": last_status}, {"$set": {"status": f"daddy sugar_{str(id_talent)}"}}
        )
    ...
    async def tambah_moans_girl(self, id_talent: int, id_bot: int, nama: str):
        last_data = {"moansgirl": self.get_data_bot(id_bot).moansgirl}
        ...
        last_status = self.get_data_pelanggan().status_full
        mycol.update_one(
            {"status": last_status}, {"$set": {"status": f"moans girl_{str(id_talent)}"}}
        )
    ...
    async def tambah_moans_boy(self, id_talent: int, id_bot: int, nama: str):
        last_data = {"moansboy": self.get_data_bot(id_bot).moansboy}
        ...
        last_status = self.get_data_pelanggan().status_full
        mycol.update_one(
            {"status": last_status}, {"$set": {"status": f"moans boy_{str(id_talent)}"}}
        )
    ...
    async def tambah_gf_rent(self, id_talent: int, id_bot: int, nama: str):
        last_data = {"gfrent": self.get_data_bot(id_bot).gfrent}
        ...
        last_status = self.get_data_pelanggan().status_full
        mycol.update_one(
            {"status": last_status}, {"$set": {"status": f"girlfriend rent_{str(id_talent)}"}}
        )
    ...
    async def tambah_bf_rent(self, id_talent: int, id_bot: int, nama: str):
        last_data = {"bfrent": self.get_data_bot(id_bot).bfrent}
        ...
        last_status = self.get_data_pelanggan().status_full
        mycol.update_one(
            {"status": last_status}, {"$set": {"status": f"boyfriend rent_{str(id_talent)}"}}
        )
